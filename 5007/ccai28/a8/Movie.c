...
Movie* CreateMovie() {
  ...
  // TODO(student): Populate/Initialize movie.
  mov->id = NULL;
  mov->type = NULL;
  mov->title = NULL;
  mov->isAdult = 0;
...
Movie* CreateMovieFromRow(char *data_row) {
  ...
  // TODO(student): Change such that genres is an array (or linkedlist),
  // not just a string.
  char* gens = token[8];
  if (gens != NULL) {
    for (int i = 0; ; i++) {
      char *gen = strtok_r(gens, ",", &gens);
      if (gen != NULL) {
        break;
      } else {
        mov->genres[i] = CheckAndAllocateString(gen);
      }
    }
  }
  return mov;
