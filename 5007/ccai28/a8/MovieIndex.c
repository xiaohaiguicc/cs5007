...
Index CreateIndex() {
  ...
  // TODO(student): How big to make this hashtable?
  // How to decide? What to think about?
...
int AddMovieTitleToIndex(Index index,
  ...
  // TODO(student): How to choose this number?
...
// Assumes index is a hashtable with key=string of the field,
// and value is a SetOfMovies
int AddMovieToIndex(Index index, Movie *movie, enum IndexField field) {
  ...
  if (result < 0) {
    ...
    switch (field) {
      ...
      case Id:
        ...
        break;
      case Genre:
        // TODO(student): What to do if there are multiple genres?
        doc_set_name = "genres";
    }
  ...
  // TODO(student): How do we put the movie in the index?
  AddMovieToSetOfMovies((SetOfMovies)kvp.value, movie);
...
uint64_t ComputeKey(Movie* movie, enum IndexField which_field) {
  switch (which_field) {
    ...
    case Id:
      ...
      break;
    case Genre:
      // TODO(student): how to deal with multiple genres??
      return -1u;
  }
  return -1u;
}
